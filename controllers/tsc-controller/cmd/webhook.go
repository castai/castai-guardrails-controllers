...
func (m *admissionMetrics) WriteText(w http.ResponseWriter) {
	...
	fmt.Fprintf(w, "tsc_admission_requests_total %d\n", requests)
	fmt.Fprintf(w, "tsc_mutations_total %d\n", mutations)
	fmt.Fprintf(w, "tsc_bypasses_total %d\n", bypasses)
	fmt.Fprintf(w, "tsc_errors_total %d\n", errs)
}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := json.Unmarshal(bodyBytes, review); err != nil ||
			...
			if json.Unmarshal(review.Request.Object.Raw, pod) == nil &&
				pod.Annotations[AnnotationBypass] == "true" {
				isBypass = true
			}
		}
